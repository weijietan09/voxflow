...
def test_hifigan_upsamples_to_hop_length():
    voc = HiFiGANLite(n_mels=16, base_channels=16, upsample_rates=(4, 4), upsample_kernel_sizes=(8, 8))
...
def test_hifigan_batched():
    voc = HiFiGANLite(n_mels=16, base_channels=16, upsample_rates=(2, 2), upsample_kernel_sizes=(4, 4))
...
def test_griffin_lim_produces_waveform():
    cfg = AudioConfig(sample_rate=16000, n_fft=256, hop_length=64, win_length=256, n_mels=32, f_max=8000)
